...
public class Handler {
	...
	public static void main(String[] args){
		...
		for(int j = 0; j<20; ++j){
			...
			int max = MAX_THREAD;
			convertThread[] threads = new convertThread[MAX_THREAD];
			if(pics.length < MAX_THREAD) max = pics.length;
			for(int i = 0; i<max; ++i){
				threads[i] = new convertThread();
				threads[i].start();
			}
			...
			while(!done){
				...
				for(int i = 0; i<max;++i){
					if(threads[i].getState() == Thread.State.TERMINATED){
						if(iter < pics.length){					
							threads[i] = new convertThread(pics[iter],String.valueOf(iter));
							threads[i].start();
							done = false;	
							iter++;	
						}
